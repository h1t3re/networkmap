...
void *accept_client_co(void *communication_fd){
  ...
  //while( (msg_size = recv(communication_fd, str, 100, 0)) != 0){//Read from comm_fd and store into str
  while(read(file_fd, &str[i_bytes], sizeof(str[i_bytes]))){
    //printf("%s", msg_size, str);
    i_bytes++;
    str = (char *)realloc(str, i_bytes*sizeof(char));
